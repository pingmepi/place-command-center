-- Phase 11: Advanced User Management - Role System Refactor & Bulk Operations
...
-- 2. Create user_roles table for flexible role assignment
...
-- 13. Update existing RLS policies to use new role system
...
FOR INSERT
TO authenticated
WITH CHECK (public.is_admin() OR public.has_role(auth.uid(), 'event_organizer'));
...
FOR UPDATE
TO authenticated
USING (public.is_admin() OR public.has_role(auth.uid(), 'event_organizer'));
...
FOR INSERT
TO authenticated
WITH CHECK (public.is_admin() OR public.has_role(auth.uid(), 'community_manager'));

DROP POLICY IF EXISTS "Admins can update discussions" ON public.discussions;
CREATE POLICY "Admins can update discussions"
ON public.discussions
FOR UPDATE
TO authenticated
USING (public.is_admin() OR public.has_role(auth.uid(), 'moderator'));
...
-- 15. Create trigger to update role timestamps
